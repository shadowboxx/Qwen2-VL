...
MODEL="/home/huyunliu/models/Qwen/Qwen2-VL-7B-Instruct" # Set the path if you do not want to load from huggingface directly
...
if [ $? -eq 0 ]; then
  ...
  echo "python openai_api.py --server-name 0.0.0.0 --server-port 8080 --checkpoint-path $OUTPUT-$(date +%Y%m%d)" > ./api.sh
  ./api.sh
fi
